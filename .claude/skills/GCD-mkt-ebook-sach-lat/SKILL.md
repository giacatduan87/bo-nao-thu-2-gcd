...
description: >
  ...
  có cấu trúc Phần–Chương + seed CTA kêu gọi hành động, (3) build PDF có bìa + logo thương hiệu của bạn + box CTA (Chrome headless),
...
Ebook trong hệ GCD là **info-product / mồi câu cao cấp**, nên bám đúng khung đã grounded của hệ:
...
- **Nghiên cứu trước, không bịa** (Luật 2a của [[GCD-AIOS-tao-skill]]): đứng trên nguồn gốc (Google Doc người dùng đưa, `raw/`, NotebookLM, WebSearch) rồi mới viết.
Cặp đôi tự nhiên với [[GCD-mkt-lead-magnet]] (chọn & outline mồi câu) → skill này **sản xuất & xuất bản** thành phẩm.

## Khi nào dùng / KHÔNG dùng
- **Dùng** khi cần ra một **ebook/sách điện tử** thành phẩm (PDF có thương hiệu) và/hoặc **sách lật Heyzine**.
- **KHÔNG dùng** cho: chỉ lên ý tưởng mồi câu (→ [[GCD-mkt-lead-magnet]]); bài blog SEO (→ [[GCD-AIOS-dang-bai-seo]]); infographic ảnh (→ [[GCD-mkt-infographic-html]]); slide.

## Tiền điều kiện
- **Node + Chrome** (build PDF). Chrome ở `C:/Program Files/Google/Chrome/Application/chrome.exe`.
- **Novamira MCP** (host PDF công khai lên <domain-cua-ban>) — xem [[hoichutuong-wordpress-novamira]].
- **Heyzine key** ở `.env` (`HEYZINE_API_KEY`, `HEYZINE_CLIENT_ID`) — xem [[heyzine-flipbook-setup]].
- **lark-cli `--as user`** nếu đăng Wiki Academy — xem [[lark-cli-setup]], [[lark-wiki-academy-gcd]].

## Quy trình thực thi (6 giai đoạn)

### 1. Nghiên cứu
Làm rõ đề bài (chủ đề · đối tượng · mục tiêu · giọng · khung sườn riêng · độ dài). Thu nguồn: doc/raw người dùng đưa → NotebookLM → WebSearch. **Phân biệt KHUNG vs NỘI DUNG** (khung sườn người dùng đưa: chỉ lấy cấu trúc). Áp [[GCD-AIOS-rebrand-doi-ten]]. Doc lớn quá token → đọc theo chunk / Grep.
...
### 5. Heyzine sách lật
```bash
bash "<thư-mục-bộ-não>/.claude/skills/GCD-mkt-ebook-sach-lat/scripts/heyzine.sh" "<public-pdf-url>"
```
→ in `SÁCH LẬT: https://heyzine.com/flip-book/<id>.html`. (Auth Bearer key + `k=client_id`, CHỈ `pdf`+`k`.)

### 6. Xuất bản & ghi sổ
(Tuỳ chọn) đăng Wiki Academy ([[lark-wiki-academy-gcd]]: `wiki +node-create` + `docs +update append`; đính PDF qua `drive +upload --wiki-token`; nhúng link sách lật). Lưu tất cả 1 thư mục output, cập nhật `index.md` + `log.md`, lưu memory nếu là tài sản dùng lại. (Tuỳ chọn) gửi link vào nhóm Zalo/Lark.
...
- **Logo**: thả logo CỦA BẠN vào `assets/` với đúng 2 tên `logo-hoichutuong-white.png` (dùng trên nền tối) và `logo-hoichutuong-dark.png` (dùng trên nền sáng). Bản phát hành không kèm logo sẵn — thiếu file thì PDF vẫn build bình thường, chỉ là không có logo.
