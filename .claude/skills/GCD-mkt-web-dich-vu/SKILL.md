...
- Landing page phễu bán **một sản phẩm/khoá** trên Ladipage → [[GCD-mkt-ladipage]] / [[GCD-mkt-leadpage]].
- Website **cá nhân/thương hiệu GCD** theo mẫu có sẵn → dùng output `web-ca-nhan-gcd` làm gốc.
- **E-commerce/bán lẻ nhiều SKU** (giỏ hàng, thanh toán) → cân nhắc WordPress/Shopify, skill này chỉ lo phần marketing site.
- Chỉ cần **infographic/ảnh** → [[GCD-mkt-infographic-html]].
...
```bash
node ".claude/skills/GCD-mkt-web-dich-vu/scripts/scaffold.mjs" --brief "duong-dan/brief.json"
# hoặc nhanh:
node ".claude/skills/GCD-mkt-web-dich-vu/scripts/scaffold.mjs" --ten "Nha khoa ABC" --slug "nha-khoa-abc" --mau "#0E7C7B" --sdt "0900000000"
```
