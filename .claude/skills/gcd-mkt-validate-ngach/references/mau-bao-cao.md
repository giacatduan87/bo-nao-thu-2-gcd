# Mẫu output — Báo cáo Markdown + file .canvas

Bản gốc BIZ.MKT.OS xuất JSON + HTML. Bộ não này thay bằng **báo cáo Markdown** (nguồn sự thật, đầy đủ evidence) + **file `.canvas`** (trực quan tóm tắt điểm). Cả hai ghi vào **cùng một thư mục kết quả** theo CLAUDE.md:

```
output/YYYY-MM-DD-validate-ngach-<ten-ngach>/
├── YYYY-MM-DD-validate-ngach-<ten-ngach>.md   # báo cáo chính
└── <Tên ngách>.canvas                          # sơ đồ điểm
```
...
## Phần 1 — Báo cáo Markdown (`YYYY-MM-DD-validate-ngach-<ten-ngach>.md`)

Copy khung dưới, điền data thật. **Không ghi đè** báo cáo cũ cho cùng ngách — thêm log mới vào "Nhật ký cập nhật".
...
created: YYYY-MM-DD
updated: YYYY-MM-DD
tags: [market-research, niche-validation]
sources: []
---

# Validate ngách — <Tên ngách>
...
> Trực quan điểm số: [[<Tên ngách>.canvas|Mở canvas]]
...
| 7 | Personal Alignment | _/13 | (neo vào trang doanh nghiệp trong `wiki/entities/`) |
|   | **TOTAL** | **_/100** | |
...
## So với doanh nghiệp hiện tại (link trang trong `wiki/entities/`)
- Ngách này khớp/lệch định vị hiện tại ở đâu:
- Key resources đang có đủ để làm không:
- (Nếu `wiki/` chưa có trang doanh nghiệp → ghi "Chưa neo được, nên chạy `gcd-sale-bmc-phong-van-chi-tiet` trước")
...
- Trang wiki cần cập nhật/thách thức: [[wiki/overview]] · [[concepts/...]] · [[entities/...]]
- Nếu số mới mâu thuẫn trang wiki cũ → ghi rõ cả hai + ngày, không âm thầm ghi đè (CLAUDE.md §3)
```
